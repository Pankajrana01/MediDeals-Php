...
class ADMIN_Controller extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->vendors = $this->config->item('admin_vendors');
	...
	protected function savevendors($name)
    {
        if ($this->vendors === true) {
            $this->load->model('Products_model');
			$name =$this>$name;
			$email =$this>$email;
			$firm_name =$this>$firm_name;
			$firm_address =$this>$firm_address;
			$drug_licence =$this>$drug_licence;
			$fssai_no =$this>$fssai_no;
			$contact_no =$this>$contact_no;
			$gst_number =$this>$gst_number;
			$user_type =$this>$user_type;
            $this->Products_model->setvendors($name,$email,$firm_name,$firm_address,$drug_licence,$fssai_no,$contact_no,$gst_number,$user_type);
        }
    }
	
	// user role
	protected function userrole1($userrole)
	{
		$menus = $this->Home_admin_model->userrolemenus($userrole);
		return $menus;
	}
